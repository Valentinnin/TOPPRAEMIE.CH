...
@app.route('/send-email-rechtsvers', methods=['POST'])
def send_email_rechtsvers():
    # Formular-Daten erfassen
    vorname = request.form.get('VorundNachname') 
    geburtsdatum = request.form.get('Geburtsdatum') 
    strassehausnr = request.form.get('StrasseHausnummer')  
    plzort = request.form.get('OrtPLZ')  
    email = request.form.get('Email')
    telefon = request.form.get('Telefonnummer') 
    aktgesellschaft = request.form.get('AktuelleGesellschaft')
    gesellschaft = request.form.get('Gesellschaft')
    aktrechtsstreitigkeit = request.form.get('AktuelleRechtstreitigkeit')
    

# Debugging-Ausgabe: Ausgabe der empfangenen Daten in der Konsole
    print(f"Formulardaten: Vorname: {vorname}, Geburtsdatum: {geburtsdatum}, E-Mail: {email}, Telefon: {telefon}")

    # E-Mail vorbereiten
    msg = Message(f'Neue Nachricht von {vorname}',
                  recipients=['lunatestmail@gmx.ch'])  # Deine E-Mail-Adresse
    msg.body = f"""
    Neue Rechtsschutz-Anfrage:

    Vorname / Name: {vorname}
    Geburtsdatum: {geburtsdatum}
    Strasse / Hausnummer: {strassehausnr}
    PLZ / Ort: {plzort}
    E-Mail: {email}
    Telefonnummer: {telefon}
    Aktuelle Gesellschaft Ja / Nein: {aktgesellschaft}
    Aktuelle Gesellschaft: {gesellschaft}
    Aktuelle Rechtsstreitigkeit: {aktrechtsstreitigkeit}
